...
void load_constant(u64 value,
  ...
  s64 svalue = value;
  if (svalue == 0) {
    gen->add_instr(gIGen->xor_gpr64_gpr64(dest_reg, dest_reg), irec);
  } else if (svalue > 0) {
    if (svalue < UINT32_MAX) {
      gen->add_instr(gIGen->mov_gpr64_u32(dest_reg, value), irec);
    } else {
      // need a real 64 bit load
      gen->add_instr(gIGen->mov_gpr64_u64(dest_reg, value), irec);
    }
  } else {
    if (svalue >= INT32_MIN) {
      gen->add_instr(gIGen->mov_gpr64_s32(dest_reg, svalue), irec);
    } else {
      // need a real 64 bit load
      gen->add_instr(gIGen->mov_gpr64_u64(dest_reg, value), irec);
    }
...
void regset_common(emitter::ObjectGenerator* gen,
  ...
  if (src_class == RegClass::GPR_64 && dst_class == RegClass::GPR_64) {
    if (src_reg == dst_reg) {
      ...
      gen->add_instr(gIGen->null(), irec);
    } else {
      gen->add_instr(gIGen->mov_gpr64_gpr64(dst_reg, src_reg), irec);
    }
    ...
    if (src_reg == dst_reg) {
      ...
      gen->add_instr(gIGen->mov_xmm32_xmm32(dst_reg, src_reg), irec);
    }
    ...
    if (src_reg == dst_reg) {
      ...
      gen->add_instr(gIGen->mov_vf_vf(dst_reg, src_reg), irec);
    }
  } else if (src_class == RegClass::FLOAT && dst_class == RegClass::GPR_64) {
    // xmm 1x -> gpr
    gen->add_instr(gIGen->movd_gpr32_xmm32(dst_reg, src_reg), irec);
    // don't forget to sign extend
    gen->add_instr(gIGen->movsx_r64_r32(dst_reg, dst_reg), irec);
  } else if (src_class == RegClass::GPR_64 && dst_class == RegClass::FLOAT) {
    // gpr -> xmm 1x
    gen->add_instr(gIGen->movd_xmm32_gpr32(dst_reg, src_reg), irec);
    ...
    gen->add_instr(gIGen->movq_xmm64_gpr64(dst_reg, src_reg), irec);
  } else if (src_is_xmm128 && dst_class == RegClass::GPR_64) {
    gen->add_instr(gIGen->movq_gpr64_xmm64(dst_reg, src_reg), irec);
  } else {
...
void IR_Return::do_codegen(emitter::ObjectGenerator* gen,
  ...
  if (val_reg == dest_reg) {
    ...
    // gen->add_instr(gIGen->mov_gpr64_gpr64(dest_reg, val_reg), irec);
  }
}
...
void IR_LoadSymbolPointer::do_codegen(emitter::ObjectGenerator* gen,
  ...
  if (m_name == "#f") {
    ...
    if (dest_reg.is_128bit_simd()) {
      gen->add_instr(gIGen->movq_xmm64_gpr64(dest_reg, gRegInfo->get_st_reg()), irec);
    } else {
      gen->add_instr(gIGen->mov_gpr64_gpr64(dest_reg, gRegInfo->get_st_reg()), irec);
    }
  } else if (m_name == "#t") {
    gen->add_instr(gIGen->lea_reg_plus_off8(dest_reg, gRegInfo->get_st_reg(),
    ...
    auto instr = gen->add_instr(
        gIGen->lea_reg_plus_off32(dest_reg, gRegInfo->get_st_reg(), 0x0afecafe), irec);
...
void IR_SetSymbolValue::do_codegen(emitter::ObjectGenerator* gen,
  ...
  auto instr = gen->add_instr(
      gIGen->store32_gpr64_gpr64_plus_gpr64_plus_s32(
...
void IR_GetSymbolValue::do_codegen(emitter::ObjectGenerator* gen,
  ...
  if (m_sext) {
    auto instr = gen->add_instr(
        gIGen->load32s_gpr64_gpr64_plus_gpr64_plus_s32(
    ...
    //     gIGen->add_gpr64_imm(gRegInfo->get_offset_reg(), LINK_SYM_NO_OFFSET_FLAG), irec);

    auto instr = gen->add_instr(
        gIGen->load32u_gpr64_gpr64_plus_gpr64_plus_s32(
            dst_reg, gRegInfo->get_st_reg(), gRegInfo->get_offset_reg(), LINK_SYM_NO_OFFSET_FLAG),
        irec);
...
void IR_GotoLabel::do_codegen(emitter::ObjectGenerator* gen,
                              ...
                              emitter::IR_Record irec) {
  (void)allocs;
  auto instr = gen->add_instr(gIGen->jmp_32(), irec);
...
RegAllocInstr IR_FunctionCall::to_rai() {
  ...
  for (int i = 0; i < emitter::gRegInfo->N_REGS; i++) {
...
void IR_FunctionCall::do_codegen(emitter::ObjectGenerator* gen,
  ...
  gen->add_instr(gIGen->add_gpr64_gpr64(freg, emitter::gRegInfo->get_offset_reg()), irec);
  gen->add_instr(gIGen->call_r64(freg), irec);
...
void IR_RegValAddr::do_codegen(emitter::ObjectGenerator* gen,
  ...
  // x86 pointer to var
  gen->add_instr(gIGen->lea_reg_plus_off(dst, SP, stack_offset), irec);
  // x86 -> GOAL pointer
  gen->add_instr(gIGen->sub_gpr64_gpr64(dst, emitter::gRegInfo->get_offset_reg()), irec);
}
...
void IR_StaticVarAddr::do_codegen(emitter::ObjectGenerator* gen,
  ...
  auto instr = gen->add_instr(gIGen->static_addr(dr, 0), irec);
  gen->link_instruction_static(instr, m_src->rec, m_src->get_addr_offset());
  gen->add_instr(gIGen->sub_gpr64_gpr64(dr, emitter::gRegInfo->get_offset_reg()), irec);
}
...
void IR_IntegerMath::do_codegen(emitter::ObjectGenerator* gen,
  ...
  switch (m_kind) {
    case IntegerMathKind::ADD_64:
      gen->add_instr(
          gIGen->add_gpr64_gpr64(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)),
          irec);
      break;
    case IntegerMathKind::SUB_64:
      gen->add_instr(
          gIGen->sub_gpr64_gpr64(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)),
          irec);
      break;
    case IntegerMathKind::AND_64:
      gen->add_instr(
          gIGen->and_gpr64_gpr64(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)),
          irec);
      break;
    case IntegerMathKind::OR_64:
      gen->add_instr(
          gIGen->or_gpr64_gpr64(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)), irec);
      break;
    case IntegerMathKind::XOR_64:
      gen->add_instr(
          gIGen->xor_gpr64_gpr64(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)),
          irec);
      break;
    case IntegerMathKind::NOT_64:
      gen->add_instr(gIGen->not_gpr64(get_reg(m_dest, allocs, irec)), irec);
      ASSERT(!m_arg);
      break;
    case IntegerMathKind::SHLV_64:
      gen->add_instr(gIGen->shl_gpr64_cl(get_reg(m_dest, allocs, irec)), irec);
      ASSERT(get_reg(m_arg, allocs, irec) == emitter::R4);
      break;
    case IntegerMathKind::SHRV_64:
      gen->add_instr(gIGen->shr_gpr64_cl(get_reg(m_dest, allocs, irec)), irec);
      ASSERT(get_reg(m_arg, allocs, irec) == emitter::R4);
      break;
    case IntegerMathKind::SARV_64:
      gen->add_instr(gIGen->sar_gpr64_cl(get_reg(m_dest, allocs, irec)), irec);
      ASSERT(get_reg(m_arg, allocs, irec) == emitter::R4);
      break;
    case IntegerMathKind::SHL_64:
      gen->add_instr(gIGen->shl_gpr64_u8(get_reg(m_dest, allocs, irec), m_shift_amount), irec);
      break;
    case IntegerMathKind::SHR_64:
      gen->add_instr(gIGen->shr_gpr64_u8(get_reg(m_dest, allocs, irec), m_shift_amount), irec);
      break;
    case IntegerMathKind::SAR_64:
      gen->add_instr(gIGen->sar_gpr64_u8(get_reg(m_dest, allocs, irec), m_shift_amount), irec);
      break;
    case IntegerMathKind::IMUL_32: {
      // just a 32-bit multiply, signed/unsigned doesn't affect lower 32 bits of result.
      auto dr = get_reg(m_dest, allocs, irec);
      gen->add_instr(gIGen->imul_gpr32_gpr32(dr, get_reg(m_arg, allocs, irec)), irec);
      // the PS2 sign extends the result even if we used multu. We replicate this here.
      gen->add_instr(gIGen->movsx_r64_r32(dr, dr), irec);
    } break;
    case IntegerMathKind::IMUL_64: {
      auto dr = get_reg(m_dest, allocs, irec);
      gen->add_instr(gIGen->imul_gpr64_gpr64(dr, get_reg(m_arg, allocs, irec)), irec);
    } break;
    case IntegerMathKind::IDIV_32: {
      gen->add_instr(gIGen->cdq(), irec);
      gen->add_instr(gIGen->idiv_gpr32(get_reg(m_arg, allocs, irec)), irec);
      gen->add_instr(gIGen->movsx_r64_r32(get_reg(m_dest, allocs, irec), emitter::R0), irec);
    } break;
    case IntegerMathKind::UDIV_32: {
      // zero extend, not sign extend to avoid overflow
      gen->add_instr(gIGen->xor_gpr64_gpr64(Register(R3), Register(R3)), irec);
      gen->add_instr(gIGen->unsigned_div_gpr32(get_reg(m_arg, allocs, irec)), irec);
    ...
    case IntegerMathKind::IMOD_32: {
      gen->add_instr(gIGen->cdq(), irec);
      gen->add_instr(gIGen->idiv_gpr32(get_reg(m_arg, allocs, irec)), irec);
      gen->add_instr(gIGen->movsx_r64_r32(get_reg(m_dest, allocs, irec), emitter::R3), irec);
    } break;
    case IntegerMathKind::UMOD_32: {
      // zero extend, not sign extend to avoid overflow
      gen->add_instr(gIGen->xor_gpr64_gpr64(Register(R3), Register(R3)), irec);
...
void IR_FloatMath::do_codegen(emitter::ObjectGenerator* gen,
  ...
  switch (m_kind) {
    case FloatMathKind::DIV_SS:
      gen->add_instr(
          gIGen->divss_xmm_xmm(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)), irec);
      break;
    case FloatMathKind::MUL_SS:
      gen->add_instr(
          gIGen->mulss_xmm_xmm(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)), irec);
      break;
    case FloatMathKind::ADD_SS:
      gen->add_instr(
          gIGen->addss_xmm_xmm(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)), irec);
      break;
    case FloatMathKind::SUB_SS:
      gen->add_instr(
          gIGen->subss_xmm_xmm(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)), irec);
      break;
    case FloatMathKind::MAX_SS:
      gen->add_instr(
          gIGen->maxss_xmm_xmm(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)), irec);
      break;
    case FloatMathKind::MIN_SS:
      gen->add_instr(
          gIGen->minss_xmm_xmm(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)), irec);
      break;
    case FloatMathKind::SQRT_SS:
      gen->add_instr(gIGen->sqrts_xmm(get_reg(m_dest, allocs, irec), get_reg(m_arg, allocs, irec)),
                     irec);
      break;
    default:
      ASSERT(false);
...
void IR_StaticVarLoad::do_codegen(emitter::ObjectGenerator* gen,
  ...
  if (m_dest->ireg().reg_class == RegClass::FLOAT) {
    ...
    auto instr = gen->add_instr(gIGen->static_load_xmm32(get_reg(m_dest, allocs, irec), 0), irec);
    ...
    // vector structure.
    auto instr = gen->add_instr(gIGen->loadvf_rip_plus_s32(get_reg(m_dest, allocs, irec), 0), irec);
    gen->link_instruction_static(instr, m_src->rec, 0);
  } else {
    ASSERT(false);
  }
...
void IR_ConditionalBranch::do_codegen(emitter::ObjectGenerator* gen,
  ...
  switch (condition.kind) {
    case ConditionKind::EQUAL:
      jump_instr = gIGen->je_32();
      break;
    case ConditionKind::NOT_EQUAL:
      jump_instr = gIGen->jne_32();
      break;
    case ConditionKind::LEQ:
      if (condition.is_signed) {
        jump_instr = gIGen->jle_32();
      } else {
        jump_instr = gIGen->jbe_32();
      }
      break;
    case ConditionKind::GEQ:
      if (condition.is_signed) {
        jump_instr = gIGen->jge_32();
      } else {
        jump_instr = gIGen->jae_32();
      }
      break;

    case ConditionKind::LT:
      if (condition.is_signed) {
        jump_instr = gIGen->jl_32();
      } else {
        jump_instr = gIGen->jb_32();
      }
      break;
    case ConditionKind::GT:
      if (condition.is_signed) {
        jump_instr = gIGen->jg_32();
      } else {
        jump_instr = gIGen->ja_32();
      }
      break;
    default:
      ASSERT(false);
  ...
  if (condition.is_float) {
    gen->add_instr(
        gIGen->cmp_flt_flt(get_reg(condition.a, allocs, irec), get_reg(condition.b, allocs, irec)),
        irec);
  } else {
    gen->add_instr(gIGen->cmp_gpr64_gpr64(get_reg(condition.a, allocs, irec),
...
void IR_LoadConstOffset::do_codegen(emitter::ObjectGenerator* gen,
  ...
  if (m_dest->ireg().reg_class == RegClass::GPR_64) {
    gen->add_instr(gIGen->load_goal_gpr(dest_reg, base_reg, emitter::gRegInfo->get_offset_reg(),
    ...
    gen->add_instr(
        gIGen->load_goal_xmm32(dest_reg, base_reg, emitter::gRegInfo->get_offset_reg(), m_offset),
        irec);
  } else if ((m_dest->ireg().reg_class == RegClass::VECTOR_FLOAT ||
    ...
    gen->add_instr(
        gIGen->load_goal_xmm128(dest_reg, base_reg, emitter::gRegInfo->get_offset_reg(), m_offset),
        irec);
  } else {
...
void IR_StoreConstOffset::do_codegen(emitter::ObjectGenerator* gen,
  ...
  if (m_value->ireg().reg_class == RegClass::GPR_64) {
    gen->add_instr(gIGen->store_goal_gpr(base_reg, value_reg, emitter::gRegInfo->get_offset_reg(),
                                         m_offset, m_size),
                   irec);
  } else if (m_value->ireg().reg_class == RegClass::FLOAT && m_size == 4) {
    gen->add_instr(
        gIGen->store_goal_xmm32(base_reg, value_reg, emitter::gRegInfo->get_offset_reg(), m_offset),
        irec);
  } else if ((m_value->ireg().reg_class == RegClass::VECTOR_FLOAT ||
              m_value->ireg().reg_class == RegClass::INT_128) &&
             m_size == 16) {
    gen->add_instr(
        gIGen->store_goal_vf(base_reg, value_reg, emitter::gRegInfo->get_offset_reg(), m_offset),
        irec);
  } else {
    throw std::runtime_error(
...
void IR_FloatToInt::do_codegen(emitter::ObjectGenerator* gen,
                               const AllocationResult& allocs,
                               emitter::IR_Record irec) {
  gen->add_instr(gIGen->float_to_int32(get_reg(m_dest, allocs, irec), get_reg(m_src, allocs, irec)),
                 irec);
  gen->add_instr(gIGen->movsx_r64_r32(get_reg(m_dest, allocs, irec), get_reg(m_dest, allocs, irec)),
                 irec);
}
...
void IR_IntToFloat::do_codegen(emitter::ObjectGenerator* gen,
                               const AllocationResult& allocs,
                               emitter::IR_Record irec) {
  gen->add_instr(gIGen->int32_to_float(get_reg(m_dest, allocs, irec), get_reg(m_src, allocs, irec)),
                 irec);
}
...
void IR_GetStackAddr::do_codegen(emitter::ObjectGenerator* gen,
  ...
  if (offset == 0) {
    gen->add_instr(gIGen->mov_gpr64_gpr64(dest_reg, SP), irec);
    gen->add_instr(gIGen->sub_gpr64_gpr64(dest_reg, gRegInfo->get_offset_reg()), irec);
  } else {
    // dest = offset + SP
    gen->add_instr(gIGen->lea_reg_plus_off(dest_reg, SP, offset), irec);
...
void IR_AsmFNop::do_codegen(emitter::ObjectGenerator* gen,
                            ...
                            emitter::IR_Record irec) {
  (void)allocs;
  gen->add_instr(gIGen->nop_vf(), irec);
}
...
void IR_AsmFWait::do_codegen(emitter::ObjectGenerator* gen,
                             ...
                             emitter::IR_Record irec) {
  (void)allocs;
  gen->add_instr(gIGen->wait_vf(), irec);
}
...
void IR_AsmPush::do_codegen(emitter::ObjectGenerator* gen,
                            ...
                            emitter::IR_Record irec) {
  if (m_use_coloring) {
    gen->add_instr(gIGen->push_gpr64(get_reg(m_src, allocs, irec)), irec);
  } else {
    gen->add_instr(gIGen->push_gpr64(get_no_color_reg(m_src)), irec);
  }
}
...
void IR_AsmPop::do_codegen(emitter::ObjectGenerator* gen,
                           ...
                           emitter::IR_Record irec) {
  if (m_use_coloring) {
    gen->add_instr(gIGen->pop_gpr64(get_reg(m_dst, allocs, irec)), irec);
  } else {
    gen->add_instr(gIGen->pop_gpr64(get_no_color_reg(m_dst)), irec);
  }
}
...
void IR_AsmSub::do_codegen(emitter::ObjectGenerator* gen,
  ...
  if (m_use_coloring) {
    gen->add_instr(
        gIGen->sub_gpr64_gpr64(get_reg(m_dst, allocs, irec), get_reg(m_src, allocs, irec)), irec);
  } else {
    gen->add_instr(gIGen->sub_gpr64_gpr64(get_no_color_reg(m_dst), get_no_color_reg(m_src)), irec);
  }
}
...
void IR_AsmAdd::do_codegen(emitter::ObjectGenerator* gen,
  ...
  if (m_use_coloring) {
    gen->add_instr(
        gIGen->add_gpr64_gpr64(get_reg(m_dst, allocs, irec), get_reg(m_src, allocs, irec)), irec);
  } else {
    gen->add_instr(gIGen->add_gpr64_gpr64(get_no_color_reg(m_dst), get_no_color_reg(m_src)), irec);
  }
}
...
void IR_JumpReg::do_codegen(emitter::ObjectGenerator* gen,
  ...
  gen->add_instr(gIGen->jmp_r64(src_reg), irec);
}
...
void IR_VFMath3Asm::do_codegen(emitter::ObjectGenerator* gen,
  ...
  switch (m_kind) {
    case Kind::XOR:
      gen->add_instr(gIGen->xor_vf(dst, src1, src2), irec);
      break;
    case Kind::SUB:
      gen->add_instr(gIGen->sub_vf(dst, src1, src2), irec);
      break;
    case Kind::ADD:
      gen->add_instr(gIGen->add_vf(dst, src1, src2), irec);
      break;
    case Kind::MUL:
      gen->add_instr(gIGen->mul_vf(dst, src1, src2), irec);
      break;
    case Kind::MAX:
      gen->add_instr(gIGen->max_vf(dst, src1, src2), irec);
      break;
    case Kind::MIN:
      gen->add_instr(gIGen->min_vf(dst, src1, src2), irec);
      break;
    case Kind::DIV:
      gen->add_instr(gIGen->div_vf(dst, src1, src2), irec);
      break;
    default:
      ASSERT(false);
  }
...
void IR_Int128Math3Asm::do_codegen(emitter::ObjectGenerator* gen,
  ...
  switch (m_kind) {
    case Kind::PEXTUB:
      ...
      gen->add_instr(gIGen->pextub_swapped(dst, src2, src1), irec);
      break;
    case Kind::PEXTUH:
      // NOTE: this is intentionally swapped because x86 and PS2 do this opposite ways.
      gen->add_instr(gIGen->pextuh_swapped(dst, src2, src1), irec);
      break;
    case Kind::PEXTUW:
      // NOTE: this is intentionally swapped because x86 and PS2 do this opposite ways.
      gen->add_instr(gIGen->pextuw_swapped(dst, src2, src1), irec);
      break;
    case Kind::PEXTLB:
      // NOTE: this is intentionally swapped because x86 and PS2 do this opposite ways.
      gen->add_instr(gIGen->pextlb_swapped(dst, src2, src1), irec);
      break;
    case Kind::PEXTLH:
      // NOTE: this is intentionally swapped because x86 and PS2 do this opposite ways.
      gen->add_instr(gIGen->pextlh_swapped(dst, src2, src1), irec);
      break;
    case Kind::PEXTLW:
      // NOTE: this is intentionally swapped because x86 and PS2 do this opposite ways.
      gen->add_instr(gIGen->pextlw_swapped(dst, src2, src1), irec);
      break;
    case Kind::PCPYLD:
      // NOTE: this is intentionally swapped because x86 and PS2 do this opposite ways.
      gen->add_instr(gIGen->pcpyld_swapped(dst, src2, src1), irec);
      break;
    case Kind::PCPYUD:
      gen->add_instr(gIGen->pcpyud(dst, src1, src2), irec);
      break;
    case Kind::PCEQB:
      gen->add_instr(gIGen->parallel_compare_e_b(dst, src2, src1), irec);
      break;
    case Kind::PCEQH:
      gen->add_instr(gIGen->parallel_compare_e_h(dst, src2, src1), irec);
      break;
    case Kind::PCEQW:
      gen->add_instr(gIGen->parallel_compare_e_w(dst, src2, src1), irec);
      break;
    case Kind::PCGTB:
      gen->add_instr(gIGen->parallel_compare_gt_b(dst, src1, src2), irec);
      break;
    case Kind::PCGTH:
      gen->add_instr(gIGen->parallel_compare_gt_h(dst, src1, src2), irec);
      break;
    case Kind::PCGTW:
      gen->add_instr(gIGen->parallel_compare_gt_w(dst, src1, src2), irec);
      break;
    case Kind::PSUBW:
      // psubW on mips is psubD on x86...
      gen->add_instr(gIGen->vpsubd(dst, src1, src2), irec);
      break;
    case Kind::POR:
      gen->add_instr(gIGen->parallel_bitwise_or(dst, src2, src1), irec);
      break;
    case Kind::PXOR:
      gen->add_instr(gIGen->parallel_bitwise_xor(dst, src2, src1), irec);
      break;
    case Kind::PAND:
      gen->add_instr(gIGen->parallel_bitwise_and(dst, src2, src1), irec);
      break;
    case Kind::PACKUSWB:
      gen->add_instr(gIGen->vpackuswb(dst, src1, src2), irec);
      break;
    case Kind::PADDB:
      gen->add_instr(gIGen->parallel_add_byte(dst, src1, src2), irec);
      break;
    default:
      ASSERT(false);
  }
...
void IR_VFMath2Asm::do_codegen(emitter::ObjectGenerator* gen,
  ...
  switch (m_kind) {
    case Kind::ITOF:
      gen->add_instr(gIGen->itof_vf(dst, src), irec);
      break;
    case Kind::FTOI:
      gen->add_instr(gIGen->ftoi_vf(dst, src), irec);
      break;
    default:
      ASSERT(false);
  }
...
void IR_Int128Math2Asm::do_codegen(emitter::ObjectGenerator* gen,
  ...
  switch (m_kind) {
    case Kind::PW_SLL:
      ...
      ASSERT(*m_imm >= 0);
      ASSERT(*m_imm <= 255);
      gen->add_instr(gIGen->pw_sll(dst, src, *m_imm), irec);
      break;
    case Kind::PW_SRL:
      ASSERT(m_imm.has_value());
      ASSERT(*m_imm >= 0);
      ASSERT(*m_imm <= 255);
      gen->add_instr(gIGen->pw_srl(dst, src, *m_imm), irec);
      break;
    case Kind::PH_SLL:
      // you are technically allowed to put values > 32 in here.
      ASSERT(m_imm.has_value());
      ASSERT(*m_imm >= 0);
      ASSERT(*m_imm <= 255);
      gen->add_instr(gIGen->ph_sll(dst, src, *m_imm), irec);
      break;
    case Kind::PH_SRL:
      ASSERT(m_imm.has_value());
      ASSERT(*m_imm >= 0);
      ASSERT(*m_imm <= 255);
      gen->add_instr(gIGen->ph_srl(dst, src, *m_imm), irec);
      break;
    case Kind::PW_SRA:
      ASSERT(m_imm.has_value());
      ASSERT(*m_imm >= 0);
      ASSERT(*m_imm <= 255);
      gen->add_instr(gIGen->pw_sra(dst, src, *m_imm), irec);
      break;
    case Kind::VPSRLDQ:
      ASSERT(m_imm.has_value());
      ASSERT(*m_imm >= 0);
      ASSERT(*m_imm <= 255);
      gen->add_instr(gIGen->vpsrldq(dst, src, *m_imm), irec);
      break;
    case Kind::VPSLLDQ:
      ASSERT(m_imm.has_value());
      ASSERT(*m_imm >= 0);
      ASSERT(*m_imm <= 255);
      gen->add_instr(gIGen->vpslldq(dst, src, *m_imm), irec);
      break;
    case Kind::VPSHUFLW:
      ASSERT(m_imm.has_value());
      ASSERT(*m_imm >= 0);
      ASSERT(*m_imm <= 255);
      gen->add_instr(gIGen->vpshuflw(dst, src, *m_imm), irec);
      break;
    case Kind::VPSHUFHW:
      ASSERT(m_imm.has_value());
      ASSERT(*m_imm >= 0);
      ASSERT(*m_imm <= 255);
      gen->add_instr(gIGen->vpshufhw(dst, src, *m_imm), irec);
      break;
    default:
      ASSERT(false);
  }
...
void IR_BlendVF::do_codegen(emitter::ObjectGenerator* gen,
  ...
  auto src1 = get_reg_asm(m_src1, allocs, irec, m_use_coloring);
  auto src2 = get_reg_asm(m_src2, allocs, irec, m_use_coloring);
  gen->add_instr(gIGen->blend_vf(dst, src1, src2, m_mask), irec);
}
...
void IR_SplatVF::do_codegen(emitter::ObjectGenerator* gen,
  ...
  auto dst = get_reg_asm(m_dst, allocs, irec, m_use_coloring);
  auto src = get_reg_asm(m_src, allocs, irec, m_use_coloring);
  gen->add_instr(gIGen->splat_vf(dst, src, m_element), irec);
}
...
void IR_SwizzleVF::do_codegen(emitter::ObjectGenerator* gen,
  ...
  auto dst = get_reg_asm(m_dst, allocs, irec, m_use_coloring);
  auto src = get_reg_asm(m_src, allocs, irec, m_use_coloring);
  gen->add_instr(gIGen->swizzle_vf(dst, src, m_controlBytes), irec);
}
...
void IR_SqrtVF::do_codegen(emitter::ObjectGenerator* gen,
  ...
  auto dst = get_reg_asm(m_dst, allocs, irec, m_use_coloring);
  auto src = get_reg_asm(m_src, allocs, irec, m_use_coloring);
  gen->add_instr(gIGen->sqrt_vf(dst, src), irec);
}
